"""
Train adaptive selection transformer on AFIB dataset (Normal vs Abnormal).
Tracks patch/head/block selection rates, FLOPs, inference time, and per-class metrics 
for comparison with fixed and adaptive halting models.
"""
import os
import time
import numpy as np
...
def main():
    ...
    dropout     = 0.1
    num_classes = 2
    gumbel_tau  = 1.0     # Gumbel-sigmoid temperature
    ...
    max_grad_norm   = 1.0
    label_smoothing = 0.0
    alpha_p         = 5e-4   # compute usage loss weight (same as alpha_p in adaptive halting)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            # compute_fraction plays the role of ponder_loss (compute usage regularization)
            loss = task_loss + alpha_p * compute_fraction
        ...
        with torch.no_grad():
            for x_val, y_val in val_loader:
                ...
                logits_val, compute_val = model(x_val)
                task_loss_val = criterion(logits_val, y_val)
                loss_val = task_loss_val + alpha_p * compute_val

                val_loss += loss_val.item() * x_val.size(0)
                val_compute += compute_val.item() * x_val.size(0)
        ...
        val_epoch_compute = val_compute / val_total

        print(
            f"==> Epoch {epoch+1}/{num_epochs} "
            f"| Train Loss: {train_epoch_loss:.4f} Train Acc: {train_epoch_acc:.4f} "
            f"| Val Loss: {val_epoch_loss:.4f} Val Acc: {val_epoch_acc:.4f} "
            f"| Val Compute: {val_epoch_compute:.4f}"
        )
    ...
    print("Training complete. Run test_selection_afib.py for test metrics.")
